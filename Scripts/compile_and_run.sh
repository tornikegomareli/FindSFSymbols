...
APP_NAME=${APP_NAME:-FindSFSymbols}
...
log "==> package app"
# A stable identity keeps the Keychain "Always Allow" valid across rebuilds. Ad-hoc signing does not.
# The Developer ID identity is the first choice, because the released app has the same signature.
if [[ -z "${APP_IDENTITY:-}" ]]; then
  APP_IDENTITY="$(security find-identity -v -p codesigning 2>/dev/null \
    | sed -n -E 's/.*"((Developer ID Application|Apple Development): [^"]+)".*/\1/p' \
    | sort -r | head -1)"
fi
if [[ -z "${APP_IDENTITY}" ]]; then
  log "WARN: no signing identity found. The build is ad-hoc signed and the Keychain asks on every rebuild."
fi
